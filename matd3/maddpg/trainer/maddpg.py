...
def q_train(make_obs_ph_n, act_space_n, q_index, q_func, optimizer, grad_norm_clipping=None, local_q_func=False, scope="trainer", reuse=None, num_units=64):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        action_grad = optimizer.compute_gradients(q, act_ph_n)
        action_grad_func = U.function(inputs=obs_ph_n + act_ph_n, outputs=action_grad)
        ...
        return train, update_target_q, {'q_values': q_values, 'target_q_values': target_q_values, 'action_grad': action_grad_func}
